package com.sophism.sampleapp;

/**
 * Created by D.H.KIM on 2016. 1. 14.
 */
public class AppDefine {

    //우체국 API key
    public static final String POST_API_KEY = "f61512ad589727fec1449213044040";
    //네이버 Login key
    public static final String OAUTH_CLIENT_ID = "jluSiGNplHYd4jkzNTxE";
    public static final String OAUTH_CLIENT_SECRET = "A1KNlrYiSh";
    public static final String OAUTH_CLIENT_NAME = "샘플 프로젝트";

    //parse key
    public static final String PARSE_APP_ID = "I8RbwKXqiwLpw6JaWRa37fjt7KuuG8skAAGLe5Hj";		//Parse.com Appplication Key
    public static final String PARSE_CLIENT_KEY = "8jPBDTHJTFiezFk322d842QBfrdCDgRqSJdjkAEJ";	//Parse.com Client Key

    //Chat Server
    public static final String CHAT_SERVER_URL = "http://chat.socket.io";
}
